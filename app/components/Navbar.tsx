...
const Navbar = ({isDarkMode, setIsDarkMode}) => {
  ...
  const openMenu = () => {
    sideMenuRef.current?.style.setProperty('transform', 'translateX(-12rem)')
  }
  ...
  return (
    ...
    {/* <div>
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20" : ""}`}>
        ...
        <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-48 top-0 bottom-0 w-48 z-50 h-screen bg-rose-50 dark:bg-gray-400 transition duration-500'>
